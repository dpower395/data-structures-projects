...
private:
    ...
    // returns appropriate leaf for insertion, nullptr if already present
    shared_ptr<btree> find_leaf(shared_ptr<btree> root, int key);
    
    // recursively splits overfull node and moves middle key up
    void move_and_split(shared_ptr<btree> curNode, shared_ptr<btree> root);
    
    // finds the parent of the current node, returns it
    shared_ptr<btree> find_parent(shared_ptr<btree> curNode, shared_ptr<btree> root);
    
    // for creating nodes
    shared_ptr<btree> initialize_node();
    shared_ptr<btree> create_node(int size, int* keys);
};
